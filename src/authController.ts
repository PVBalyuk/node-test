...
class authController {
  async registration(req: Request, res: Response) {
    try {
      ...
      const u = await Customer.findOne({ where: { firstName: req.body.firstName } });
      if (u) {
  ...
  async login(req: Request, res: Response) {
    try {
      ...
      if (!validPass) return res.status(400).json('Неверный пароль');
  ...
  async RefreshAccessToken(req: Request, res: Response) {
    try {
      ...
      const decoded: Dec = jwt_decode(refreshTokenFromDB);
